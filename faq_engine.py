...
class FAQEngine:
    ...
    def _check_conversational_intent(self, query):
        """
        Detects common conversational intents (greetings, language requests, small talk, gratitude, farewells)
        in Hinglish, Urdu, and English.
        Returns (answer_string, confidence_score) or (None, None).
        """
        if not query:
            return None, None
        ...
        hinglish_triggers = [
            "hinglish", "roman urdu", "hinglish me", "hinglish ma", 
            "hinglish bolo", "hinglish me baat", "hinglish ma baat", 
            "speak in hinglish", "talk in hinglish", "urdu me baat kro", 
            "urdu ma baat kro", "urdu me bolo", "urdu ma bolo"
        ]
        if any(trigger in clean_text for trigger in hinglish_triggers):
            return "Ji bilkul! Ab se main aap se asaan Hinglish (Roman Urdu) me baat karunga. Aap mujh se koi bhi sawal pooch sakte hain — main asani se samjhaoon ga!", 1.0

        pure_urdu_triggers = ["اردو میں بات", "اردو بولو", "اردو میں بولو", "speak in urdu"]
        if any(trigger in query for trigger in pure_urdu_triggers):
            return "جی بالکل! اب سے میں آپ سے اردو میں بات کروں گا۔ آپ اپنا سوال پوچھ سکتے ہیں، میں رہنمائی کے لیے حاضر ہوں!", 1.0

        english_triggers = ["speak in english", "talk in english", "english please", "english me bolo", "english ma bolo"]
        if any(trigger in clean_text for trigger in english_triggers):
            return "Certainly! I will communicate with you in English. Please feel free to ask any questions you have!", 1.0
        
        # 2. Greetings
        greetings = {
            "hi", "hello", "hey", "hy", "heyy", "salam", "assalam", 
            "assalamoalaikum", "assalamualaykum", "aoa", "greetings", "hola"
        }
        if clean_text in greetings or (len(tokens) <= 2 and any(token in greetings for token in tokens)):
            return "Hello! Salam! Main aapka friendly AI Assistant hoon. Aap mujh se Hinglish, Urdu, ya English kisi bhi zubaan me sawal pooch sakte hain. Kaise madad karoon aapki?", 1.0
            
        # 3. How are you / Hal chal
        how_are_you = [
            "how are you", "how are you doing", "how r u", "how do you do", 
            "whats up", "wassup", "sup", "hows it going", 
            "kya hal hai", "kya haal hai", "kese ho", "kaise ho", "kya chal raha", "sab theek"
        ]
        if any(h in clean_text for h in how_are_you):
            return "Main bilkul theek-thaak aur aapki madad ke liye ready hoon! Aap batayein, aaj main aapki kya help kar sakta hoon?", 1.0
            
        # 4. Identity / Who are you
        identity = [
            "who are you", "what is your name", "whats your name", "who made you", "what are you",
            "aap kaun ho", "tum kaun ho", "kon ho tum", "apna naam btao", "naam kya hai"
        ]
        if any(i in clean_text for i in identity):
            return "Main ek smart AI FAQ Chatbot Assistant hoon! Main Hinglish, Urdu aur English teeno me asani se baat kar sakta hoon aur aapke sawalat solve karta hoon.", 1.0
            
        # 5. Gratitude / Shukriya
        gratitude = [
            "thank you", "thanks", "thx", "thank u", "thank you so much",
            "shukriya", "bohot shukriya", "bahut shukriya", "jazakallah", "mehrbani"
        ]
        if any(g in clean_text for g in gratitude):
            return "You're most welcome! Aapka bohot shukriya! Agar koi aur sawal ho to bila-jhijhak pooch lijiye ga.", 1.0
            
        # 6. Farewells / Allah Hafiz
        farewells = [
            "bye", "goodbye", "see you", "cya", "bye bye", "good night",
            "allah hafiz", "khuda hafiz", "alvida", "chalta hu", "chalta hoon"
        ]
        if any(f in clean_text for f in farewells):
            return "Allah Hafiz / Goodbye! Apna bohot khayal rakhiye ga, jab bhi koi sawal ho wapas zaroor aayiye ga!", 1.0
            
        # 7. Jokes / Mazahiya baat
        jokes = ["tell me a joke", "joke", "make me laugh", "tell joke", "koi joke sunao", "lateefa sunao", "mazaq"]
        if any(j in clean_text for j in jokes):
            return "Ek programmer ne doosre se poocha: 'Tum dark mode kyun use karte ho?'\nDoosre ne kaha: 'Kyunki roshni (light) se bugs attract hote hain!' 😄", 1.0

        # 8. Capabilities / Help
        capabilities = [
            "what can you do", "help me", "how can you help", "what do you do",
            "kya kar sakte ho", "kya kr skty ho", "madad chahiye", "help chahiye", "kya help kar sakte ho"
        ]
        if any(c in clean_text for c in capabilities):
            return "Main aapko Technology (AI, Python, Cloud), Customer Support, Order Tracking, Account Settings, aur Education ke bare me asaan Hinglish, Urdu, ya English me samjha sakta hoon!", 1.0
    ...
    def get_response(self, user_query, threshold=0.3):
        ...
        intent_ans, intent_score = self._check_conversational_intent(user_query)
        if intent_ans:
            return intent_ans, intent_score, []
